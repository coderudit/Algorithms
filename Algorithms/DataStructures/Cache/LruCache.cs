namespace Algorithms.DataStructures.Cache;
...
public class LruCache<TKey, TValue> where TKey: notnull
{
    ...
    /// <summary>
    /// <c>Dictionary</c> stores <c>LinkedListNode</c> as it allows
    /// removing the node from the <c>LinkedList</c> in O(1) time.
    /// </summary>
    private readonly Dictionary<TKey, LinkedListNode<CachedItem>> _cache = new();
    private readonly LinkedList<CachedItem> _lruList = new();
    ...
    public void Put(TKey key, TValue value)
    {
        ...
        var newNode = _lruList.AddLast(item);
        _cache.Add(key, newNode);
    }

    private void UpdateLru(LinkedListNode<CachedItem> node)
    {
